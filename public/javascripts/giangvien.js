(function (namespace, $) {
    ...
    // events
    p._initNghiencuu = function () {
        $('#form-them-nghiencuu').validate({
            submitHandler: function (form) {
                materialadmin.AppOffcanvas._handleOffcanvasClose();
                // $.ajax({
                //     type: 'POST',
                //     url: '/users/giangvien/nghiencuu',
                //     data: $(this).serialize()
                // }).done(function (data) {
                //     console.log(data);
                //     if (data.success) {
                //         var nghiencuu = data.item;
                //         var html = '';
                //         html += '<li class="tile">';
                //         html += '     <a class="tile-content ink-reaction">';
                //         html += '         <div class="tile-text">';
                //         html += nghiencuu.chude;
                //         html += '<small>' + nghiencuu.mota + '</small>';
                //         html += '         </div>';
                //         html += '     </a>';
                //         html += '<a class="btn btn-flat ink-reaction btn-default btn-remove-nghiencuu"><i class="fa fa-trash"></i></a>';
                //         html += '</li>';
                //         $('#list-nghiencuu').append(html);
                //     }
                // });
                return false;
            }
        ...
        $('.btn-remove-nghiencuu').on('click', function (e) {
            var self = $(this);
            var text = self.prev().find('p').text().trim();
            if (confirm('Xóa nghiên cứu này?')) {
                $.ajax({
                    type: 'DELETE',
                    url: '/users/giangvien/nghiencuu/' + text,
                    dataType: 'json'
                }).done(function (data) {
                    if (data.success) {
    ...
    p._initLinhvuc = function () {
        $('.linhvuc_lienquan input:checkbox').on('change', function () {
            $('#btn_linhvuc_lienquan').show();
        });

        $("#linhvuc_form").on("submit", function (event) {
            ...
            $.ajax({
                type: 'POST',
                url: '/users/giangvien/linhvuc',
                data: $(this).serialize(),
                dataType: 'json'
            }).done(function (data) {
                console.log(data);
                if (data.success) {
                    $('#btn_linhvuc_lienquan').hide();
                }
            }).always(function () {
